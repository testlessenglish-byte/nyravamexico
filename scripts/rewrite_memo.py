...
with open('src/lib/export-legal-memo.ts', 'r', encoding='utf-8') as f:
    text = f.read()

imports = """import { capturePdfText } from "./reporting/rendered-output";
...
export function downloadLegalMemoPdf(payload: FinalReportPayload, caseName: string, caseData: any = {}): void {
  ...
  if ((facts.chronology?.length ?? 0) + (facts.undisputed?.length ?? 0) + (facts.disputed?.length ?? 0) > 0) {
    ...
    if ((facts.chronology?.length ?? 0) > 0) {
      ...
      b.table([rt("Date"), rt("Event"), rt("Source")], rows, [60, 200, 100]);
    }
  ...
  if (risks.length > 0) {
    ...
    b.table([rt("Risk"), rt("Probability"), rt("Impact"), rt("Mitigation")], rows, [100, 80, 80, 100]);
  }
  ...
  if (actions.length > 0) {
    ...
    b.table([rt("Priority"), rt("Action"), rt("Owner"), rt("Deadline")], rows, [60, 150, 80, 80]);
  }
...
with open('src/lib/export-legal-memo.ts', 'w', encoding='utf-8') as f:
    f.write(imports + body)
print("Rewritten export-legal-memo.ts completely.")
