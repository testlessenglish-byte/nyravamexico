...
with open('original_audit.ts', 'r', encoding='utf-8') as f:
    text = f.read()

imports = """import { rt } from "../../report-i18n";
import { PdfBuilder } from "../../export";
import { translateLegalTerm } from "../../pdf/enum-translation";
"""

# replace jsPDF import with PdfBuilder imports
text = re.sub(r'import jsPDF from "jspdf";\nimport autoTable from "jspdf-autotable";', imports, text)

# Now rewrite generateAuditReportPdf
new_gen = """export function generateAuditReportPdf(data: AuditReportData): Uint8Array {
...
text = re.sub(r'export function generateAuditReportPdf\(data: AuditReportData\): Uint8Array \{.*?^\}', new_gen, text, flags=re.DOTALL|re.MULTILINE)

with open('src/lib/social/reports/audit-report-pdf.server.ts', 'w', encoding='utf-8') as f:
    f.write(text)
print('Rewrote audit report with proper parameters and PdfBuilder.')
